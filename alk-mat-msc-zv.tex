...
\begin{equation*}
	\mathbf{ M } = \begin{bmatrix}
		\mathbf{ B }\\
		- \mathbf{ I }_{ E }
	\end{bmatrix} \qquad \Rightarrow \qquad \begin{array}{rcl}
...
\begin{description}
	...
	\item[Maximális vágás problémája és duálisa] Legyen $ G = \left( V,{} E \right) $ egyszerű gráf.
		...
		Egy $ \mathcal{ V } $ vágás: $ \mathbf{ x } \in \left\{ -1,{} 1 \right\}^{ V } \subset \mathbb{ R }^{ V } $, ahol a komponens $ - 1 $, ha a megfelelő csúcs a vágás egyik, $ 1 $, ha a vágás másik oldalára esik. Legyen $ \mathbf{ A } $ a gráf szomszédsági mátrixa. Az $ \mathbf{ x }^{ \T } \mathbf{ A } \mathbf{ x } $ kvadratikus alakban az $ e = \left\{ u,{} v \right\} $ él hozzájárulása $ 2 x_{ u } x_{ v } $, $ x_{ u } x_{ v } = 1 $, ha $ e $-t \textqq{nem vágtuk ketté}, $ - 1 $ különben. A feladat és duálisa:
		\begin{equation*}
	 		\begin{array}{rcl}
				x_{ v }^{ 2 } 									&		=		& 1 \quad \left( \forall \, v \in V \right)\\
				\hline
	...
	\item[Maximális független ponthalmaz felső becslése]  Legyen $ G = \left( V,{} E \right) $
		...
		nevezzük, és nyilván $ \alpha \left( G \right) \le \theta \left( G \right) $.
